...
const AuthProviderCard = ({ provider, onClick, disabled = false }) => (
  ...
  >
    <span className={`mx-auto grid h-16 w-16 place-items-center rounded-full ${logoBgClasses[provider.logoBg] || logoBgClasses.neutral}`}>
      {provider.logoUrl ? (
        <img src={provider.logoUrl} alt="" className="h-9 w-9 object-contain" loading="lazy" referrerPolicy="no-referrer" />
      ) : (
