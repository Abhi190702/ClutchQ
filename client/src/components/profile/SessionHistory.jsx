...
const SessionHistory = ({ sessions = [] }) => (
  <div className="card p-5">
    <h3 className="mb-4 text-lg font-black text-clutch-text">Session history</h3>
        ...
        sessions.map((session) => (
          <div key={session._id} className="rounded-lg border border-clutch-border bg-clutch-panelSoft p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="text-xs text-clutch-muted">{shortDateTime(session.startedAt)}</div>
            </div>
            <div className="mt-1 text-sm text-clutch-muted">Chemistry {session.chemistryScore || 0}% · {session.result}</div>
          </div>
        ))
      ) : (
